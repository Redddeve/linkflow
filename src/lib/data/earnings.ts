...
async function fetchPayoutRows(filters: EarningsFilters): Promise<EarningsListRow[]> {
  ...
  let query = supabase
    .from('orders')
    .select(
      'id, site_domain, published_at, publish_date, sourcer_payout_cents, sourcer_paid_at, sourcer_payout_reference, sites!inner(sourcer_id)',
    )
  ...
  return rows
    .map((r): EarningsListRow | null => {
      ...
      return {
        ...
        sourcer_payout_cents: r.sourcer_payout_cents ?? 0,
...
export async function sumUnpaidPayouts(): Promise<number> {
  ...
  const { data } = await supabase
    .from('orders')
    .select('sourcer_payout_cents, sites!inner(sourcer_id)')
