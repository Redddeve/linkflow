...
export async function addToCart(input: AddCartItemInput): Promise<void> {
  ...
  if (error) {
    if (error.code === '23505')
      throw new AppError('CONFLICT', 'Already in cart');
...
export async function checkoutCart(): Promise<{ orderIds: string[] }> {
  const actor = await requireRole(['Client']).catch(mapForbidden);
  ...
  if (!cart) throw new AppError('VALIDATION', 'Cart is empty');
  ...
  for (const item of items) {
    ...
    if (site.status !== 'Active') {
      throw new AppError(
        'VALIDATION',
        `Site "${site.domain}" is no longer active`,
      );
    }
    if (!item.publish_date) {
      throw new AppError(
        'VALIDATION',
        `Publish date is required for "${site.domain}"`,
      );
    }
    if (new Date(item.publish_date) < today) {
      throw new AppError(
        'VALIDATION',
        `Publish date for "${site.domain}" is in the past`,
      );
    }
  }
  ...
  if (ordersError || !orders)
    throw new Error(ordersError?.message ?? 'Failed to create orders');
