...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 9, color: '#666', textTransform: 'uppercase', marginBottom: 4 },
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View style={styles.meta}>
            ...
            <Text style={styles.metaValue}>{formatBillingMonth(invoice.billing_month)}</Text>
        ...
        <View style={styles.table}>
          ...
          {orders.map((o) => (
            <View style={styles.row} key={o.id}>
              ...
              <Text style={styles.cellPrice}>${(o.price_cents / 100).toFixed(2)}</Text>
            </View>
          ))}
        </View>
        ...
        {invoice.status === 'Sent' && invoice.sent_at && (
          <View style={styles.stamp}>
            <Text>SENT — {new Date(invoice.sent_at).toLocaleDateString('en-CA')}</Text>
          </View>
        )}
        {invoice.status === 'Paid' && invoice.marked_as_paid_at && (
          <View style={[styles.stamp, { borderColor: '#080', color: '#080' }]}>
            <Text>PAID — {new Date(invoice.marked_as_paid_at).toLocaleDateString('en-CA')}</Text>
          </View>
        )}
